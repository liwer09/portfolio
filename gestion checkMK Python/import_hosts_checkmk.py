...
def service_discovery(line):
    ...
    requests.post("https://checkmk.central.cirsa.com/master/check_mk/api/v0/domain-types/service_discovery_run/actions/start/invoke", headers=headers, data=body, verify=False)

#Funcion aplicar cambios
def apply_changes():
    global headers
    requests.post("https://checkmk.central.cirsa.com/master/check_mk/api/v0/domain-types/activation_run/actions/activate-changes/invoke", headers=headers, verify=False)
...
def crearhost(line):
    ...
    "tag_agent": agente, 
    "tag_snmp_ds": snmp, 
    "tag_criticality": line[10],
    "tag_TipoDispositivo": str(line[5]),
    "tag_Fabricante": str(line[6]),
    "tag_Modelo": str(line[7]),
    "tag_SistemaOperativo": str(line[8]),
    "tag_Servicio": str(line[9])
    }})
    #enviamos la request
    requests.post('https://checkmk.central.cirsa.com/master/check_mk/api/1.0/domain-types/host_config/collections/all', verify=False, headers=headers, data=data)
